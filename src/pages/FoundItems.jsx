...
export const FoundItems = () => {
  ...
  const items = MOCK_ITEMS.filter(i => i.type === 'found');
  ...
  return (
    <div className="page">
      <div className="items-page">
        <div className="items-page-header" style={{ background: 'linear-gradient(135deg, var(--found-bg), var(--paper))' }}>
          <div className="container items-page-header-inner">
            <div>
              <p className="section-eyebrow" style={{ color: 'var(--found)' }}>✅ Found Items</p>
              <h1 className="items-page-title">Items Waiting to Go Home</h1>
              <p className="items-page-subtitle">{items.length} items found by the community</p>
            </div>
            <div className="items-page-search">
              <SearchBar placeholder="Search found items..." onSearch={setSearch} large />
            </div>
          </div>
        </div>
        ...
        <div className="container items-layout">
          <aside className="items-sidebar">
            <div className="filter-panel">
              ...
              <div className="filter-group">
                ...
                <div className="filter-options">
                  ...
                  {CATEGORIES.slice(0, 8).map(cat => (
                    <button key={cat} className={`filter-chip ${category === cat ? 'filter-chip-active' : ''}`} onClick={() => setCategory(cat === category ? '' : cat)}>
                      {cat}
                    </button>
                  ))}
                </div>
              ...
              {(search || category) && (
                <Button variant="ghost" size="sm" onClick={() => { setSearch(''); setCategory(''); }}>Clear filters</Button>
              )}
            </div>
          </aside>
          ...
          <main className="items-main">
            <div className="items-results-header">
              <p className="items-count"><span className="items-count-num">{filtered.length}</span> items found</p>
            </div>
            {filtered.length > 0 ? (
              ...
              <div className="items-empty">
                ...
                <Button variant="secondary" onClick={() => { setSearch(''); setCategory(''); }}>Reset filters</Button>
              </div>
            )}
          </main>
        </div>
